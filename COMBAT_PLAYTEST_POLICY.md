...
- **同一ターゲット・同一防具・満 HP に近い状態**で撃ち、試行回数を増やして **分布**を見る（**`ignoresArmor` を厳密に 0 にしない**。下記「実機メモ」。正の値同士の差なら鎧が効く条件ほど出やすい）。
...
- **アイテム説明の「装甲無視」段**は **`0.0` にしても高段のまま**（下記 **0.1 追試**でも高段のまま、という報告あり。素体 `ignoresArmor` と一致しない可能性）。
...
`short_bow` の **`primaryWeaponMode.ignoresArmor` を `0.1`** にした場合:

- **攻撃は可能**（`0.0` のときの「ダメージを与えられない」ゲートは掛からない）。
- **アイテム側の「装甲貫通」段の表記は `0.35` のときと同様、高段のまま**だった、という実機報告がある。
