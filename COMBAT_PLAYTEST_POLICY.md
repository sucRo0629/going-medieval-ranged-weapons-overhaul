...
- **アイテム説明の「装甲無視」段**は **`0.0` にしても高段のまま**（別テーブル・段階閾値・またはキャッシュ等で、素の数値と一致しない可能性）。
...
**方針**: `ignoresArmor` の切り分けは **`0` を避け**、**小さな正の値同士**（例: `0.10` vs `0.35`）か、**別武器・別素体**の比較で行う。`Equipment.json` の **`short_bow` 素体は設計上 `0.35` を正**とする（プレイテスト用の一時 `0.0` はコミットしない）。
