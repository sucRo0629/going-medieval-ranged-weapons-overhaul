...
- **Base 4.5 境界と鋼（Steel）上限**: `CombatScore(Base)` は原則 **4.5** を上限とし、金属武器は鋼製でゲームバランスの頂点（T5 最上位）に位置するよう素体値を調整する。
...
- `Constructables/ProductionComponentsRepository.json` は **バニラを上書きする前提**で扱い、Mod 側に **各作業台で作成可能にする装備 `id` を全て記載**する。差分最小（オーファンのみ追記）方針は採用しない。
  **補足（作業台）**: バニラでは **`Production` があっても**、`Constructables/ProductionComponentsRepository.json` の各作業台 `productions` に **`id` が載っていなければ UI 上は作れない**。本 Mod では上書き整合を優先し、`Production` / `Research` の有無だけでなく **作業台 `productions` の全量記載**で作成可否を管理する。洗い出しは `[scripts/vanilla_production_station_audit.py](../../scripts/vanilla_production_station_audit.py)`（`GM_STREAMING_ASSETS` または `GOING_MEDIEVAL_ITEMS` の親を `StreamingAssets` とする）。
